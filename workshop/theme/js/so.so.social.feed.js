...
(function($) {
	$.fn.soSoSocial = function(callback) {
		...
		sss.CONTAINER = $(this);
		//Move twitter to other container 
		sss.TWITTER_CONTAINER = $('#twitter');
		...
		************************************************************************************/
		...
		//$.getJSON("http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20json%20where%20url%3D%22"+encodeURIComponent(sss.TWITTER_RSS)+"%22&format=json&diagnostics=true&callback=?", function(d) {
		// Twitter
		if (sss.TWITTER_RSS != "") {
			$.getJSON(sss.TWITTER_RSS, function(d) {
				//grab ever rss item from the json result request
				console.dir(d);
				if (d.statuses) {
					$(d.statuses).each(function() {
                        ...
                        var img_link = this.user.profile_image_url;
						...
						status = status.replace(/\B@([_a-z0-9]+):/ig, "");
                                                var twitter_username = this.user.screen_name.split('@');
                                                
                                                 

						//append to the div
						sss.ACTIVITY_ARRAY[sss.COUNT] = new Array();
						sss.ACTIVITY_ARRAY[sss.COUNT][0] = '<li style="background: url(' + img_link  + ' ) no-repeat left center;">' + status + ' <a href="' + link + '" target="_blank">Leia</a>';
						sss.ACTIVITY_ARRAY[sss.COUNT][1] = relative_time_twitter(pubDate);
						sss.ACTIVITY_ARRAY[sss.COUNT][2] = get_delta_twitter(pubDate);
						sss.ACTIVITY_ARRAY[sss.COUNT][3] = 'TWITTER'
						sss.COUNT++;
					});
			...
			$.getJSON("http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20xml%20where%20url%3D%22"+encodeURIComponent(sss.LASTFM_RSS)+"%22&format=json&callback=?", function(d) {
				...
				if (d.query.results.rss) {
					$(d.query.results.rss.channel.item).each(function() {
						...
						//append to the div
						sss.ACTIVITY_ARRAY[sss.COUNT] = new Array();
						sss.ACTIVITY_ARRAY[sss.COUNT][0] = '<li><a href="' + link + '" target="_blank">' + title + '</a>';
						sss.ACTIVITY_ARRAY[sss.COUNT][1] = '<span>' + relative_time(pubDate) + '</span>';
						//sss.ACTIVITY_ARRAY[sss.COUNT][1] = relative_time(pubDate);
						sss.ACTIVITY_ARRAY[sss.COUNT][2] = get_delta(pubDate);
						sss.ACTIVITY_ARRAY[sss.COUNT][3] = 'LASTFM'
						sss.COUNT++;
					});
			...
			$.getJSON("http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20xml%20where%20url%3D%22"+encodeURIComponent(sss.FACEBOOK_RSS)+"%22&format=json&callback=?", function(d) {
				...
				if (d.query.results.rss) {
					$(d.query.results.rss.channel.item).each(function() {
						...
						//append to the div
						sss.ACTIVITY_ARRAY[sss.COUNT] = new Array();
						sss.ACTIVITY_ARRAY[sss.COUNT][0] = '<li>Posted <a href="' + link + '" target="_blank">' + title + '</a>';
						sss.ACTIVITY_ARRAY[sss.COUNT][1] = relative_time(pubDate);
						sss.ACTIVITY_ARRAY[sss.COUNT][2] = get_delta(pubDate);
						sss.ACTIVITY_ARRAY[sss.COUNT][3] = 'FACEBOOK'
						sss.COUNT++;
					});
			...
			$.getJSON("http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20xml%20where%20url%3D%22"+encodeURIComponent(sss.FLICKR_RSS)+"%22&format=json&callback=?", function(d) {
				...
				if (d.query.results.rss) {
					$(d.query.results.rss.channel.item).each(function() {
						...
						sss.ACTIVITY_ARRAY[sss.COUNT][1] = relative_time(pubDate);
						sss.ACTIVITY_ARRAY[sss.COUNT][2] = get_delta(pubDate);
						sss.ACTIVITY_ARRAY[sss.COUNT][3] = 'FLICKR'
						sss.COUNT++;
					});
		...
		if (sss.DELICIOUS_RSS != '') {
			$.getJSON("http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20xml%20where%20url%3D%22"+encodeURIComponent(sss.DELICIOUS_RSS)+"%22&format=json&callback=?", function(d) {
				...
				if (d.query.results.rss) {
					$(d.query.results.rss.channel.item).each(function() {
						//if set up to be infinite or the limit issss.ACTIVITY_ARRAY[j][0] not reached, keep grabbing items
						var title = this.title;
						var link = this.link;
						var description = this.description;
						var pubDate = this.pubDate;
						...
						sss.ACTIVITY_ARRAY[sss.COUNT][1] = relative_time(pubDate);
						sss.ACTIVITY_ARRAY[sss.COUNT][2] = get_delta(pubDate);
						sss.ACTIVITY_ARRAY[sss.COUNT][3] = 'DELICIOUS'
						sss.COUNT++;
					});
		...
		if (sss.TUMBLR_RSS != '') {
			$.getJSON("http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20xml%20where%20url%3D%22"+encodeURIComponent(sss.TUMBLR_RSS)+"%22&format=json&callback=?", function(d) {
				...
				$(d.query.results.rss.channel.item).each(function() {
					...
					sss.ACTIVITY_ARRAY[sss.COUNT][1] = relative_time(pubDate);
					sss.ACTIVITY_ARRAY[sss.COUNT][2] = get_delta(pubDate);
					sss.ACTIVITY_ARRAY[sss.COUNT][3] = 'THUMBLR'
					sss.COUNT++;
				});
			...
			$.getJSON("http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20xml%20where%20url%3D%22"+encodeURIComponent(sss.POSTEROUS_RSS)+"%22&format=json&callback=?", function(d) {
				...
				if (d.query.results.rss) {
					$(d.query.results.rss.channel.item).each(function() {
						...
						sss.ACTIVITY_ARRAY[sss.COUNT][1] = relative_time(pubDate);
						sss.ACTIVITY_ARRAY[sss.COUNT][2] = get_delta(pubDate);
						sss.ACTIVITY_ARRAY[sss.COUNT][3] = 'POSTEROUS'
						sss.COUNT++;
					});
...
function print_array(obj) {
	...
	if ((sss.FLICKR_FINISHED == 1) && (sss.TWITTER_FINISHED == 1) && (sss.FACEBOOK_FINISHED == 1) && (sss.LASTFM_FINISHED == 1) && (sss.DELICIOUS_FINISHED == 1) && (sss.TUMBLR_FINISHED == 1) && (sss.WORDPRESS_FINISHED == 1)) {
		...
		for (j = 0; j < sss.LIMIT; j++) {
                        // Move twitter content to other div.
                        if (sss.ACTIVITY_ARRAY[j][3] == "TWITTER") {
			     html2 += sss.ACTIVITY_ARRAY[j][0] + ' (' + sss.ACTIVITY_ARRAY[j][1] + ')</li>';
			} else {
		...
		sss.CONTAINER.append(html);
                sss.TWITTER_CONTAINER.append(html2);
...
function get_delta_twitter(time_value) {
	var values = time_value.split(" ");
...
function relative_time_twitter(time_value) {
	
	var delta = get_delta_twitter(time_value);

	if (delta < 60) {
		return 'menos de um minuto atrás';
	} else if(delta < 120) {
		return 'mais ou menos um minuto atrás';
	} else if(delta < (60*60)) {
		return (parseInt(delta / 60)).toString() + ' minutos atrás';
	} else if(delta < (120*60)) {
		return 'mais ou menos uma hora atrás';
	} else if(delta < (24*60*60)) {
		return 'mais ou menos ' + (parseInt(delta / 3600)).toString() + ' horas atrás';
